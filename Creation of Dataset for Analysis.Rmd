...
library(haven); library(dplyr); library(tidyr); library(readr); library(readxl); library(data.table); library(stringr); library(purrr); library(stringr)  # Data manipulation & import
...
FORt <- FORt %>%
  mutate(
    ADHDMedNonStim = ifelse(
      rowSums(across(all_of(med_vars), ~ str_detect(., "tomoxetine|strattera|guanfacine|intuniv|tenex|clonidine|kapvay|viloxazine|qelbree")),
              na.rm = TRUE) > 0, 1, 0
    )
  )
...
FORt <- FORt %>%
  mutate(
    ADHDMedStim = ifelse(
      rowSums(across(all_of(med_vars), ~ str_detect(.,"ritalin|concerta|metadate|methylin|daytrana|focalin|adderall|bifentin|vyvanse|vyvance|vivance|dexedrine|procentra|zenzedi")),
              na.rm = TRUE) > 0, 1, 0
    )
  )
...
FORt <- FORt %>%
  mutate(
    ADHDMed = ifelse(ADHDMedStim == 1|ADHDMedNonStim == 1, 1, 0)
  )
...
##melatonin
##'vitamin/vitamins', fish oil, Creatine, sleep aid, iron supplement
FORt <- FORt %>%
  mutate(
    VitOrSupp = ifelse(
      rowSums(across(all_of(med_vars),
                     ~ str_detect(., "melatonin|gummies|vitamin|iron supplement|creatine|sleep aid|fish oil")),
              na.rm = TRUE) > 0, 1, 0
    )
  )
...
FORt <- FORt %>%
  mutate(
    insulin = ifelse(
      rowSums(across(all_of(med_vars), ~ str_detect(., "insulin")),
              na.rm = TRUE) > 0, 1, 0
    )
  )
...
FORt <- FORt %>%
  mutate(
    BirthControl = ifelse(
      rowSums(across(all_of(med_vars), ~ str_detect(., "bc|birth control|birthcontrol|contraceptives")),
              na.rm = TRUE) > 0, 1, 0
    )
  )
...
FORt <- FORt %>%
  mutate(
    Analgesic = ifelse(
      rowSums(across(all_of(med_vars), ~ str_detect(., "advil|tylenol")),
              na.rm = TRUE) > 0, 1, 0
    )
  )
...
#Dev
list <- c(FORt$med1_pick, FORt$med2_pick, FORt$med3_pick, FORt$med4_pick, FORt$med5_pick, FORt$med6_pick, FORt$med7_pick)

#Remove empty strings
list <- list[ !str_detect(list, "^$") ]
list <- list[!str_detect(list, "n/a")]

#Remove vitamin and supplements
##Id assume magn is a supllmenet
list <- list[!str_detect(list, "melatonin|iron supplement|gummies|vitamin|multivitimin|multivitimans|multivit|multivitmain|supplement|iron supplement|creatine|sleep aid|fish oil|fishoil|multivitiman|b12|b-12|b complex|omega|omga|preworkout|probiotic|protein|multi|mutil|mutitivtamins|mealtonin|gummy|kids vitaamin|iron|magnesium|vit d|vit c|viatmin")]

#Birth Control
list <- list[!str_detect(list, "bc|birth control|birthcontrol|contraceptive|iud")]

#Insulin / Diabetes Treatment?
list <- list[!str_detect(list, "insulin|insullin|metformin")]

#Analgesic
list <- list[!str_detect(list, "tylenol|advil")]

#Antihistamine
list <- list[!str_detect(list, "reactine|aerius|antihistamine")]

#Inhaler
list <- list[!str_detect(list, "puffer|inhaler|ventolin|pulmacort|avamys|ventalin")]

#Gastro
list <- list[!str_detect(list, "prevacid|acid|tums")]

#Fever
list <- list[!str_detect(list, "ceterizine|allergy|allegra")]

#Acne 
list <- list[!str_detect(list, "acne|accutane")]

#Antibiotic
list <- list[!str_detect(list, "antibiotic")]

##Medication
#ADHD
list <- list[!str_detect(list, "tomoxetine|strattera|guanfacine|intuniv|tenex|clonidine|kapvay|viloxazine|qelbree")]
list <- list[!str_detect(list,"ritalin|concerta|metadate|methylin|daytrana|focalin|adderall|adderal|adderral|aderall|bifentin|vyvanse|vyvance|vivance|dexedrine|procentra|zenzedi|foquest|floquest|biphentin|adhd")]


#SSRI
list <- list[!str_detect(list, "prozac|sarafem|zoloft|paxil|luvox|celexa|lexapro|sertraline")]
head(sort(list), 1000)



#Antipsychotics
list <- list[!str_detect(list,"haldol|trilafon|thorazine|prolixin|permitil|trilafon|etrafon|mellaril|loxitane|adasuve|orap|abilify|abilify_discmelt|saphris|rexulti|vraylar|clozaril|fazaclo|versacloz|fanapt|latuda|zyprexa|zyprexa_zydis|invega|seroquel|risperdal|geodon|caplyta")]
